...
{
    symbol_t *nowscansym=symtab->next;
    while(nowscansym!=NULL){
        if(strcmp(nowscansym->name,name)==0){
            return nowscansym;
        }
        nowscansym=nowscansym->next;
    }
    return NULL;
}
...
int add_symbol(char *name)
{
    ...
    newsymbol->name=(char*)malloc(sizeof(char)*MAX_INSLEN);
...
void add_reloc(char *name, bin_t *bin)
{
    ...
    newreloc->name=(char*)malloc(sizeof(char)*MAX_INSLEN);
    strcpy(newreloc->name,name);
    newreloc->y64bin=bin;
...
parse_t parse_reg(char **ptr, regid_t *regid)
{
    ...
    reg_t *regname=find_register(*ptr);
    if (regname==NULL){
        err_print("Invalid REG");
        return PARSE_ERR;
    }
...
parse_t parse_symbol(char **ptr, char **name)
{
    ...
    char *psym=*ptr;
    int symlen=0;
    while(*psym!=','&&!IS_END(psym)&&!IS_BLANK(psym)&&!IS_COMMENT(psym)){
        symlen++;
        psym++;
    }
    char *tmp=(char*)malloc(sizeof(char)*(symlen+1));
    strncpy(tmp,*ptr,symlen);
    *(tmp+symlen)='\0';
    bin_t *binloc=(bin_t*)name;
    add_reloc(tmp,binloc);
    free(tmp);
    /* set 'ptr' and 'name' */
    *ptr+=symlen;
    return PARSE_SYMBOL;
...
parse_t parse_digit(char **ptr, long *value)
{
    ...
    *value=strtoull(*ptr,NULL,0);
    ...
    //err_print("%d",*value);
    while((**ptr<='9'&&**ptr>='0')||(**ptr<='z'&&**ptr>='a'&&base==16)||(**ptr<='Z'&&**ptr>='A'&&base==16)){
        *ptr+=1;
    }
    SKIP_BLANK(*ptr);
    //err_print("%s",*ptr);
...
parse_t parse_label(char **ptr, char **name)
{
    ...
    char *lname = (char *)malloc(sizeof(char)*(label_length+1));
    strncpy(lname,*ptr,label_length);
    *(lname+label_length)='\0';
    if (find_symbol(lname)){
        err_print("Dup symbol:%s",lname);
        return PARSE_ERR;
    }
    add_symbol(lname);
...
*           call SUM  #invoke SUM function */
    ...
    if (parse_label(&pnow,&label)==-1){
        line->type=TYPE_ERR;
        return line->type;
    }
    if(IS_END(pnow)||IS_COMMENT(pnow)){
        line->type=TYPE_INS;
    ...
    case I_NOP:
        break;
    case I_RRMOVQ:
        if(parse_reg(&pnow,&regA)==-1){
            line->type=TYPE_ERR;
            return line->type;
        }
        if(parse_delim(&pnow,',')==-1){
            line->type=TYPE_ERR;
            return line->type;
        }
        if(parse_reg(&pnow,&regB)==-1){
            line->type=TYPE_ERR;
            return line->type;
        }
        ...
        break;
    case I_IRMOVQ:
        imme=(long)&line->y64bin;
        if(parse_imm(&pnow,&symbolname,&imme)==-1){
            line->type=TYPE_ERR;
            return line->type;
        }
        if(parse_delim(&pnow,',')==-1){
        ...
        for (int i=0;i<8;i++){
            line->y64bin.codes[i+2]=(imme>>i*8)&0xff;
        }
        break;
    case I_RMMOVQ:
        if (parse_reg(&pnow,&regA)==-1){
    ...
    case I_JMP:
    case I_CALL:
        if (!IS_LETTER(pnow)&&(*(pnow)!='0'||*(pnow+1)!='x')){
        ...
        for (int i=0;i<8;i++){
            line->y64bin.codes[i+1]=(imme>>i*8)&0xff;
        }
        break;
    case I_RET:
        break;
    ...
    case I_DIRECTIVE:
        ...
        switch(LOW(instr->code)){
            case D_DATA:
                if(IS_LETTER(pnow)){
                    imme = (long)&line->y64bin;
			        parse_imm(&pnow, &symbolname, &imme);
                }
                for (int i=0;i<instr->bytes;i++){
                    line->y64bin.codes[i]=(imme>>i*8)&0xff;
                }
                break;
            case D_POS:
                vmaddr=imme;
                line->y64bin.addr=imme;
                break;
            case D_ALIGN:
                vmaddr=vmaddr%imme==0?vmaddr:vmaddr-vmaddr%imme+imme;
                line->y64bin.addr=vmaddr;
                break;
            default:
                line->type=TYPE_ERR;
...
int relocate(void)
{
    ...
    while (rtmp) {
        /* find symbol */
        symbol_t *symloc=find_symbol(rtmp->name);
        if (symloc==NULL){
        ...
        int nowbyte=0;
        if (HIGH(rtmp->y64bin->codes[0])==I_DIRECTIVE){
            nowbyte=0;
        }else{
            nowbyte=rtmp->y64bin->bytes-8;
        }
        for (int i=nowbyte;i<rtmp->y64bin->bytes;i++){
            rtmp->y64bin->codes[i]=(symloc->addr>>(i-nowbyte)*8)&(0xff);
        }
...
int binfile(FILE *out)
{
    ...
    long nowaddr=0;
    char ch[MAX_INSLEN];
    memset(ch,0,MAX_INSLEN);
    while(nowline){
        if (nowline->type==TYPE_COMM||nowline->y64bin.bytes==0){
            nowline=nowline->next;
            continue;
        }
        if (nowline->y64bin.addr>nowaddr){
            fwrite(ch,1,nowline->y64bin.addr-nowaddr,out);
            nowaddr=nowline->y64bin.addr;
        }
        fwrite(nowline->y64bin.codes,1,nowline->y64bin.bytes,out);
        nowaddr+=nowline->y64bin.bytes;
        nowline=nowline->next;
    }
