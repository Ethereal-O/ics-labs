...
stat_t nexti(y64sim_t *sim)
{
    ...
    case I_RRMOVQ: /* 2:x regA:regB */
    {
        ...
        reg_t regA = reg_table[GET_REGA(regs)];
        reg_t regB = reg_table[GET_REGB(regs)]; // get id from file
        next_pc++;
        long_t argA = get_reg_val(sim->r, regA.id); // get value from reg
        set_reg_val(sim->r, regB.id, argA);
        sim->pc = next_pc;
        }else{
            next_pc ++;
            sim->pc = next_pc;
    ...
    case I_IRMOVQ: /* 3:0 F:regB imm */
    {
        ...
        next_pc += 8;
        set_reg_val(sim->r, regB.id, imme);
        sim->pc = next_pc;
        break;
    }
    case I_RMMOVQ: /* 4:0 regA:regB imm */
    {
        ...
        next_pc += 8;
        long_t argA = get_reg_val(sim->r, regA.id); // get value from reg
        long_t argB = get_reg_val(sim->r, regB.id); // get value from reg
        long_t addr = compute_alu(A_ADD, argB, imme);
        set_long_val(sim->m, addr, argA);
        sim->pc = next_pc;
        break;
    }
    case I_MRMOVQ: /* 5:0 regB:regA imm */
    {
        ...
        long_t addr = compute_alu(A_ADD, argB, imme);
        long_t res = 0;
        if (!get_long_val(sim->m, addr, &res))
        {
            err_print("PC = 0x%lx, Invalid data address 0x%lx", sim->pc,addr);
            return STAT_ADR;
        }
        set_reg_val(sim->r, regA.id, res);
        sim->pc = next_pc;
        break;
    }
    case I_ALU: /* 6:x regA:regB */
    {
        ...
        long_t argA = get_reg_val(sim->r, regA.id); // get value from reg
        long_t argB = get_reg_val(sim->r, regB.id); // get value from reg
        long_t res = compute_alu(ifun, argA, argB);
        sim->cc = compute_cc(ifun, argA, argB, res);
        set_reg_val(sim->r,regB.id,res);
        sim->pc = next_pc;
        break;
    }
    case I_JMP: /* 7:x imm */
    ...
    case I_CALL: /* 8:x imm */
    {
        ...
        next_pc += 8;
        long_t addr = get_reg_val(sim->r, (regid_t)4) - 8;
        set_reg_val(sim->r, (regid_t)4, addr);
        if(addr<=0){
            err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc,addr);
            return STAT_ADR;
        }
        sim->pc=next_pc;
        set_long_val(sim->m, addr, sim->pc);
        sim->pc = imme;
        break;
    }
    case I_RET: /* 9:0 */
    {
        long_t nowaddr = get_reg_val(sim->r, (regid_t)4);
        long_t addr = nowaddr + 8;
        if (!get_long_val(sim->m, nowaddr, &next_pc))
        {
            err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc,nowaddr);
            return STAT_ADR;
        }
        sim->pc=next_pc;
        set_reg_val(sim->r, (regid_t)4, addr);
    ...
    case I_POPQ: /* B:0 regA:F */
    {
        ...
        reg_t regA = reg_table[GET_REGA(regs)];
        next_pc++;
        long_t addr = get_reg_val(sim->r, (regid_t)4);
        long_t res;
        if (!get_long_val(sim->m, addr, &res))
        {
            err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc,addr);
